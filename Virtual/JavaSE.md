...
二进制补码形式
[数据在计算机中的存储形式和运算](https://blog.csdn.net/LonelyRoamer/article/details/7670869?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.nonecase&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.nonecase)
...
[基本数据类型和引用数据类型的区别](https://www.cnblogs.com/maskwolf/p/9972982.html)
...
封装、继承以及多态，其中方法的重写都和多态有关。 多态的主要特征就是父类引用指向子类对象，生活中的例子：Animal animal = new Dog();

## String 为啥不可变？String 能继承吗？

Sting 是这样定义的：public final class String extends Object，里边有 final 关键字，所以不可变也不能被继承，同时 string 底层是字符数组也是 final 修饰，这样做首先是**安全**，比如 hashset 中用 string 做为键，不会出现 string 变化，导致违反唯一键。另外**节约内存**,常量池里的string可以重复利用
...
5. notify():唤醒等待队列中的其中一个线程；notifyAll():唤醒线程等待队列中的所有线；
6. wait(long timeout):让一个线程等待一段时间。
7. finalize：该方法用于释放资源
8. notifyAll方法：该方法唤醒在该对象上等待的所有线程。
9. hashcode方法：
...
参数 | 抽象类 | 接口
---|--- |---
默认的方法实现 | 它可以有默认的方法实现 | 接口完全是抽象的。它根本不存在方法的实现(JDK8之后又default修饰的方法体)
实现 | 子类使用extends关键字来继承抽象类。如果子类不是抽象类的话，它需要提供抽象类中所有声明的方法的实现。 |子类使用关键字implements来实现接口。它需要提供接口中所有声明的方法的实现
构造器 | 抽象类可以有构造器 | 接口不能有构造器
与正常Java类的区别 |除了你不能实例化抽象类之外，它和普通Java类没有任何区别|接口是完全不同的类型
访问修饰符 |抽象方法可以有public、protected和default这些修饰符|接口方法默认修饰符是public。你不可以使用其它修饰符
main方法|抽象方法可以有main方法并且我们可以运行它|接口没有main方法，因此我们不能运行它。（JDK8以后接口可以有default和static方法，所以可以运行main方法）
...
## 局部内部类和匿名内部类为什么访问final修饰的局部变量
根本原因是内部类和局部变量的生命周期不同导致的.
内部类是类级别的,他不会因为定义在方法里就随着方法的执行完毕而销毁,因此可以出现方法执行完毕销毁,而内部类未销毁,内部类访问了一个不存在的局部变量.  
为了解决这个问题,内部类会把局部变量变成自己的成员变量,表面上是方法中的局部变量,实际是访问的是备份,这样就必须保持两者一致.  
**在jdk1.8之后,即使不加声明,默认是fanal,如果修改就会报错**
...
1. hash%length==hash&(length-1)的前提是length是2的n次方,计算机中直接求余效率不如位移运算,能够提升性能
2. 能均匀分布减少碰撞,2的n次方实际就是1后面n个0，2的n次方-1  实际就是n个1,当和长度进行与的时候,能更加的均衡.
...
3. HashMap 的迭代器是Iterator迭代器,HashTable使用Enumeration
4. 数组初始化和扩容不同,HashTable中hash数组默认大小是11，增加的方式是 arr*2+1。HashMap中hash数组的默认大小是16，而且一定是2的指数。
...
3. Vector 属于遗留容器， 已经不推荐使用，vector是同步访问的,并且有部分方法不属于集合框架。
...
- AtomicLong原理：AtomicLong的原理是依靠底层的cas来保障原子性的更新数据，在要添加或者减少的时候，会使用死循环不断地cas到特定的值，从而达到更新数据的目的
- LongAdder的实现原理：首先它有一个基础的值base，在发生竞争的情况下，会有一个Cell数组用于将不同线程的操作离散到不同的节点上去(会根据需要扩容，最大为CPU核数，即最大同时执行线程数)，sum()会将所有Cell数组中的value和base累加作为返回值。核心的思想就是将AtomicLong一个value的更新压力分散到多个value中去，从而降低更新热点。所以在激烈的锁竞争场景下，LongAdder性能更好。
...
## IO 和 NIO、AIO
- NIO：同步非阻塞式IO，一个线程在同步的进行轮询检查，Selector不断轮询注册在其上的Channel，某个Channel上面发生读写连接请求，这个Channel就处于就绪状态，被Selector轮询出来，然后通过SelectionKey可以获取就绪Channel的集合，进行后续的I/O操作
- AIO：异步非阻塞式的IO，AIO框架在windows下使用windows IOCP技术，在Linux下使用epoll多路复用IO技术模拟异步IO， 即：应用程序向操作系统注册IO监听，然后继续做自己的事情。操作系统发生IO事件，并且准备好数据后，在主动通知应用程序，触发相应的函数（这就是一种以订阅者模式进行的改造）。由于应用程序不是“轮询”方式而是订阅-通知方式，所以不再需要selector轮询，由channel通道直接到操作系统注册监听。


## 怎么打印日志？

## 运行时异常与一般异常有何异同？
- RuntimeException如：NullPointerException、ClassCastException；一个是检查异常CheckedException，如I/O错误导致的IOException、SQLException。 RuntimeException是那些可能在 Java 虚拟机正常运行期间抛出的异常的超类。 如果出现RuntimeException，那么一定是程序员的错误.
...
页表具有逻辑地址到物理地址映射的作用。
![](https://img-blog.csdn.net/20170502124303856?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvcXFfMzQxMjc5NTg=/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

段表分为段号和段内地址，各16位，段内地址分为基址和段长。
![](https://img2018.cnblogs.com/blog/1020179/201904/1020179-20190428222619508-1473035710.png)

## 页面置换算法
1. 最佳
>理想状态,所选择替换出来的都是最长时间不访问的
2. 最近最久未使用(LRU)
3. 最近未使用(NRU)
>标志位R和M,访问后将R和M置1,并定时将R清零,优先清理数字小的页面
4. 先进先出(FIFO)
5. 第二次机会算法
> FIFO+标志位,每次访问的页面的R置1,替换时检查,若为零直接替换,若为1,放到链表的末尾,将R置零.
6. 时钟
